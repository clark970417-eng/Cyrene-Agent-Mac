// 小米 MiMo TTS 引擎（從桌面端 src/main/tts/mimo-engine.ts 搬移，純 fetch，無 Electron 依賴）
...
export interface MimoSynthesizeOptions {
  ...
  stylePrompt?: string;
  /** MiMo output encoding. xiaogpt/tetos requires MP3 because it parses every file as MP3. */
...
export async function synthesize(opts: MimoSynthesizeOptions): Promise<MimoSynthesizeResult> {
  ...
  const outputFormat = opts.outputFormat ?? "wav";
